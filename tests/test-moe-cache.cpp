...
static void configure_cache(const char * fail_stage) {
    ...
    set_env("GGML_CUDA_MOE_CACHE_MIN_SLOTS", "1");
...
static bool warm_direct_expert(
        const char * name, const void * base, size_t expert_size,
        int64_t direct_n_in, int64_t direct_n_out,
        int direct_type, int64_t direct_n_expert, int32_t expert) {
    for (int attempt = 0; attempt < 160; attempt++) {
        if (direct_plan_one(
                name, base, expert_size, direct_n_in, direct_n_out,
                direct_type, direct_n_expert, expert) == 1) {
            return true;
        }
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
    }
...
static bool run_small_pool_liveness(
        ggml_backend_t cuda, ggml_backend_t cpu, ggml_tensor * weights) {
    configure_cache(nullptr);
    set_env("GGML_CUDA_MOE_CACHE_BUDGET_MB", "1");
    set_env("GGML_CUDA_MOE_CACHE_MIN_SLOTS", "8");
    void * session = create_direct_session(cuda, cpu);
    if (!session) {
        fprintf(stderr, "cache-small-pool: failed to create session\n");
        return false;
    }
...
static bool run_partition_policy(
        ggml_backend_t cuda, ggml_backend_t cpu,
        log_capture & capture) {
    configure_cache(nullptr);
    set_env("GGML_CUDA_MOE_CACHE_BUDGET_MB", "2");
    set_env("GGML_CUDA_MOE_CACHE_INSERTS", "8");
    set_env("GGML_CUDA_MOE_CACHE_ADMIT_AFTER", "1");
    set_env("GGML_CUDA_MOE_CACHE_THROTTLE", "2");
    capture.clear();

    void * session = create_direct_session(cuda, cpu);
    if (!session) {
        fprintf(stderr, "cache-partition-policy: failed to create session\n");
        return false;
    }

    constexpr int64_t partition_n_in = 1024;
    constexpr int64_t partition_n_out = 144;
    constexpr int64_t partition_n_expert = 32;
    constexpr int n_layers = 4;
    constexpr int fair_share = 4;
    const size_t expert_size =
        ggml_row_size(GGML_TYPE_Q4_0, partition_n_in) * partition_n_out;
    std::vector<std::vector<uint8_t>> weights;
    std::vector<std::string> names;
    weights.reserve(n_layers);
    names.reserve(n_layers);
    for (int layer = 0; layer < n_layers; layer++) {
        weights.emplace_back(expert_size * partition_n_expert);
        names.push_back(
                "blk." + std::to_string(layer) + ".ffn_up_exps.weight");
    }

    ggml_moe_cache.session_enter(session);
    bool ready = false;
    // Discover every layer before the shape stabilizes so the initial quota
    // calculation includes all four partitions.
    for (int round = 0; round < 80 && !ready; round++) {
        for (int layer = 0; layer < n_layers; layer++) {
            ready |= direct_begin_ready(
                    names[layer].c_str(), weights[layer].data(), expert_size,
                    partition_n_in, partition_n_out,
                    GGML_TYPE_Q4_0, partition_n_expert);
        }
    }

    bool ok = ready;
    // Layer 0 may borrow all free slots while the other partitions are idle.
    for (int32_t expert = 0; expert < 16 && ok; expert++) {
        ok &= warm_direct_expert(
                names[0].c_str(), weights[0].data(), expert_size,
                partition_n_in, partition_n_out,
                GGML_TYPE_Q4_0, partition_n_expert, expert);
    }

    // Each newly active layer reclaims its fair share from the borrower.
    for (int layer = 1; layer < n_layers && ok; layer++) {
        for (int32_t expert = 0; expert < fair_share && ok; expert++) {
            ok &= warm_direct_expert(
                    names[layer].c_str(), weights[layer].data(), expert_size,
                    partition_n_in, partition_n_out,
                    GGML_TYPE_Q4_0, partition_n_expert, expert);
    ...
    // A sequential scan in layer 0 must recycle only layer 0's own slots.
    // A cache-wide LRU would evict all hot entries from layers 1 and 2 here.
    for (int32_t expert = 0; expert < 12 && ok; expert++) {
        ok &= warm_direct_expert(
                names[0].c_str(), weights[0].data(), expert_size,
                partition_n_in, partition_n_out,
                GGML_TYPE_Q4_0, partition_n_expert, expert);
    ...
    for (int layer = 1; layer < n_layers && ok; layer++) {
        for (int32_t expert = 0; expert < fair_share && ok; expert++) {
            ok &= direct_plan_one(
                    names[layer].c_str(), weights[layer].data(), expert_size,
                    partition_n_in, partition_n_out,
                    GGML_TYPE_Q4_0, partition_n_expert, expert) == 1;
        }
    }
    ...
    ggml_moe_cache.session_destroy(session);
    const std::string log = capture.get();
    ok &= max_field_value(log, "slots=") == 16;
    ok &= max_field_value(log, "partitions=") == n_layers;
    ok &= has_positive_field(log, "local=");
    ok &= has_positive_field(log, "reclaim=");
    if (!ok) {
        fprintf(stderr, "cache-partition-policy diagnostics:\n%s", log.c_str());
    }
    printf("cache-partition-policy: %s\n", ok ? "OK" : "FAIL");
    return ok;
}

static bool run_prefill_stream(
        ggml_backend_t cuda, ggml_backend_t cpu, ggml_tensor * weights,
        log_capture & capture) {
    constexpr int64_t prefill_tokens = 8;
    ggml_init_params params = {
    ...
    ggml_tensor * ids = ggml_new_tensor_2d(
            ctx, GGML_TYPE_I32, n_used, prefill_tokens);
    ggml_tensor * activations = ggml_new_tensor_3d(
            ctx, GGML_TYPE_F32, n_in, 1, prefill_tokens);
    ...
    int32_t ids_data[n_used * prefill_tokens];
    for (int token = 0; token < prefill_tokens; token++) {
        ids_data[token * n_used + 0] = token % 4;
        ids_data[token * n_used + 1] = (token + 1) % 4;
    }
    std::vector<float> activation_data(ggml_nelements(activations));
    for (size_t index = 0; index < activation_data.size(); index++) {
        activation_data[index] =
            0.3f * std::sin((float)index * 0.013f) -
            0.1f * std::cos((float)index * 0.029f);
    }
    ...
    set_env("GGML_CUDA_MOE_PREFETCH_BUDGET_MB", nullptr);
    bool ok = ggml_backend_graph_compute(cpu, graph.graph) == GGML_STATUS_SUCCESS;
    ...
    capture.clear();
    set_env("GGML_CUDA_MOE_PREFETCH_BUDGET_MB", nullptr);
    set_env("GGML_CUDA_MOE_CACHE_STATS", nullptr);
    set_env("GGML_CUDA_MOE_CACHE_MIN_CC", "0");
    ggml_backend_t backends[] = {cuda, cpu};
    ggml_backend_sched_t scheduler = ggml_backend_sched_new(
            backends, nullptr, 2, GGML_DEFAULT_GRAPH_SIZE, false, false);
    if (!scheduler) {
        ...
        ggml_backend_sched_set_moe_streaming(scheduler, 0, 4, 1);
        ggml_backend_sched_set_tensor_backend(scheduler, graph.out, cpu);
        ok &= ggml_backend_sched_alloc_graph(scheduler, graph.graph);
        if (ok) {
            ok &= ggml_backend_sched_graph_compute(scheduler, graph.graph) ==
                  GGML_STATUS_SUCCESS;
        }
        if (ok) {
            ggml_backend_tensor_get(
                    graph.out, actual.data(), 0, actual.size() * sizeof(float));
            ok &= compare_output(reference, actual, 5e-4);
        }
        ggml_backend_sched_free(scheduler);
    }
    const std::string log = capture.get();
    ok &= log.find("[moe-prefetch]") != std::string::npos;
    ok &= has_positive_field(log, "prefill=");
    printf("cache-prefill-stream: %s\n", ok ? "OK" : "FAIL");

    set_env("GGML_CUDA_MOE_CACHE_MODE", nullptr);
    free_graph(graph);
    ggml_backend_buffer_free(buffer);
    ggml_free(ctx);
    return ok;
...
int main() {
    ...
    ok &= run_small_pool_liveness(cuda, cpu, weights);
    ok &= run_shape_liveness(cuda, cpu);
    ok &= run_route_override(cuda_device, cuda, cpu);
    ok &= run_admission_policy(cuda, cpu, capture);
    ok &= run_partition_policy(cuda, cpu, capture);
    ok &= run_prefill_stream(cuda, cpu, weights, capture);
