...
void ggml_backend_buffer_free(ggml_backend_buffer_t buffer) {
    ...
    if (buffer->iface.get_base) {
        ...
        if (base) {
            ggml_backend_moe_cache_invalidate_buffer(buffer, base, ggml_backend_buffer_get_size(buffer));
        }
    }
...
void ggml_backend_buffer_clear(ggml_backend_buffer_t buffer, uint8_t value) {
    ...
    if (buffer->iface.get_base) {
        ggml_backend_moe_cache_invalidate_buffer(buffer, ggml_backend_buffer_get_base(buffer), buffer->size);
    }
...
bool ggml_backend_buffer_copy_tensor(const struct ggml_tensor * src, struct ggml_tensor * dst) {
    ...
    if (dst_buf->iface.cpy_tensor) {
        ggml_backend_moe_cache_invalidate_buffer(dst_buf, dst->data, ggml_nbytes(dst));
...
void ggml_backend_tensor_set_async(ggml_backend_t backend, struct ggml_tensor * tensor, const void * data, size_t offset, size_t size) {
    ...
    if (backend->iface.set_tensor_async == NULL) {
        ...
        ggml_backend_moe_cache_invalidate_buffer(buf, (const char *) tensor->data + offset, size);
...
void ggml_backend_tensor_set_2d_async(ggml_backend_t backend, struct ggml_tensor * tensor, const void * data, size_t offset, size_t size,
    ...
    ggml_backend_buffer_t buf = tensor->view_src ? tensor->view_src->buffer : tensor->buffer;
    ggml_backend_moe_cache_invalidate_buffer(buf, (const char *) tensor->data + offset, (n_copies - 1)*stride_tensor + size);
